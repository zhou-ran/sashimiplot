...
if sys.version_info[0] != 3:
    print("\nYou are using python" + str(sys.version_info[0]) + '.' + str(
        sys.version_info[1]) + " This verion of RSeQC needs python3!\n", file=sys.stderr)
    sys.exit()
...
def genebody_percentile(refbed, mRNA_len_cut=100):
    ...
    for line in open(refbed, 'r'):
        ...
        chrom, strand, mystat.percentile_list(gene_all_base))  # get 100 points from each gene's coordinates
...
def Rcode_write(dataset, file_prefix, format='pdf', colNum=100):
    ...
    if len(names) >= 3:
        print('data_matrix' + ' <- matrix(c(' + ','.join(names) + '), byrow=T, ' + 'ncol=' + str(colNum) + ')',
              file=ROUT)
        ...
        print(
            'heatmap(data_matrix' + ', scale=c(\"none\"),keep.dendro=F, labRow = rowLabel ' + ',Colv = NA,Rowv = NA,labCol=NA,col=cm.colors(256),margins = c(6, 8),ColSideColors = rc,cexRow=1,cexCol=1,xlab="Gene body percentile (5\'->3\')", add.expr=x_axis_expr <- axis(side=1,at=c(%s),labels=c(%s)))' % (
            ','.join([str(i) for i in tick_pos]), ','.join(['"' + str(i) + '"' for i in tick_lab])), file=ROUT)
    ...
    if len(names) == 1:
        ...
        print("legend(0,1,fill=icolor[%d:%d], legend=c(%s))" % (
        1, len(names), ','.join(["'" + str(n) + "'" for n in names])), file=ROUT)
...
def main():
    ...
    for bamfile in bamfiles:
        ...
        if file_container.count(tmp) == 0:
            ...
            print(tmp + '.' + str(file_container.count(tmp)) + '\t' + '\t'.join([str(cvg[k]) for k in sorted(cvg)]),
                  file=OUT1)
